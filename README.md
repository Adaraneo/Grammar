...
DTN ortografie (`d/t/n + e → dě/tě/ně`) řešena odděleně v `CzechOrtographyService.ApplyDTNEndingOrthography`.
...
`CzechAlternationRuleEvaluator` implementuje `ShouldShortenGenitivePlural` — čte příznak `hasGenitivePluralShortening` z valenčního slovníku nebo přímo z requestu. Registrovaná lemmata: `kráva`, `síla`, `lípa`, `houba`, `žíla`.
...
| Oblast | Stav |
|---|---|
| Iotace (labiály + `ě`) | Identifikováno, zatím neimplementováno |
| `GuessGenderAndPattern` / `GuessVerbAspect` | Stubováno — volající musí dodat vzor a vid explicitně |
| Numeralia | Nenačato |
...
### Blízký horizont
- [ ] Implementovat `CzechAlternationRuleEvaluator` plně (`ShouldShortenVowel`, `ShouldLengthenVowel`)
- [ ] SQLite-backed `IValencyProvider` jako záměna za `JsonValencyProvider`
- [ ] Odblokovat `GuessGenderAndPattern` a `GuessVerbAspect` po dokončení valenčního slovníku
- [ ] Plná podpora iotace (`IJotationRuleEvaluator` + `ApplyJotation`)
